...
async def mandate_register(payload: MandateRegisterRequest, client: StarMFClient = Depends(get_client)):
    return await client.post("v2/mandate_register", payload.model_dump(exclude_none=True))


@router.post("/cancel")
async def mandate_cancel(payload: MandateCancelRequest, client: StarMFClient = Depends(get_client)):
    return await client.post("v2/mandate_cancel", payload.model_dump(exclude_none=True))


@router.post("/get")
async def mandate_get(payload: MandateGetRequest, client: StarMFClient = Depends(get_client)):
    return await client.post("v2/mandate_get", payload.model_dump(exclude_none=True))


@router.post("/list")
async def mandate_list(payload: MandateListRequest, client: StarMFClient = Depends(get_client)):
    return await client.post("v2/mandate_list", payload.model_dump(exclude_none=True))


@router.post("/update")
async def mandate_update(payload: MandateUpdateRequest, client: StarMFClient = Depends(get_client)):
    return await client.post("v2/mandate_update", payload.model_dump(exclude_none=True))


@router.post("/link")
async def mandate_link(payload: MandateLinkRequest, client: StarMFClient = Depends(get_client)):
    return await client.post("v2/link_mandate", payload.model_dump(exclude_none=True))


@router.post("/delink")
async def mandate_delink(payload: MandateDelinkRequest, client: StarMFClient = Depends(get_client)):
    return await client.post("v2/mandate_delink", payload.model_dump(exclude_none=True))
