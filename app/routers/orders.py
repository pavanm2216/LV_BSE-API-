...
async def order_new(payload: OrderNewRequest, client: StarMFClient = Depends(get_client)):
    ...
    return await client.post("v2/order_new", payload.model_dump(exclude_none=True))


@router.post("/update")
async def order_update(payload: OrderUpdateRequest, client: StarMFClient = Depends(get_client)):
    return await client.post("v2/order_update", payload.model_dump(exclude_none=True))


@router.post("/list")
async def order_list(payload: OrderListRequest, client: StarMFClient = Depends(get_client)):
    return await client.post("v2/order_list", payload.model_dump(exclude_none=True))


@router.post("/get")
async def order_get(payload: OrderGetRequest, client: StarMFClient = Depends(get_client)):
    return await client.post("v2/order_get", payload.model_dump(exclude_none=True))


@router.post("/cancel")
async def order_cancel(payload: OrderCancelRequest, client: StarMFClient = Depends(get_client)):
    return await client.post("v2/order_cancel", payload.model_dump(exclude_none=True))
